 ...
 * assert what `0.159.0` actually does, so a version bump that changes it fails
 * here rather than in a rendered page.
 */

const repoRoot = new URL("../", import.meta.url);

/** The version this package was proven against: peer floor and dev pin. */
const PINNED = "0.159.0";
...
describe("the experimental surface stays behind one module", () => {
  ...
  it("declares the proven foldkit as the peer floor and dev pin", () => {
    ...
    expect(manifest).toMatchObject({
      peerDependencies: { foldkit: `>=${PINNED}` },
      devDependencies: { foldkit: PINNED },
    });
    expect(installed).toMatchObject({ version: PINNED });
  });
});

describe("what foldkit 0.159.0 renders, and what it leaves out", () => {
